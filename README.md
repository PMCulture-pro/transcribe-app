...
|------------|----------|--------------|------------|
| `MODEL` | `large`, `turbo` | `large` | `turbo` (large-v3-turbo) ~3× быстрее при сопоставимом качестве. Нужна turbo-модель: переустановить с `WITH_TURBO=1`. Если не скачана — откат на `large` |
